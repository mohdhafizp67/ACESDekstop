...
class LeaderboardController extends Controller
{
  public function viewLeaderboard()
  {
    ...
    $student = DB::table('students')->select(DB::raw('students_games.student_point,students_quizes.percentage,leaderboard.scores(students_games.student_point + students_quizes.percentage + leaderboard.scores) as total_points, students.id as id, users.profile_picture as profile_picture, users.name as name, users.school as school, users.state as state'))
      ...
      ->leftJoin("leaderboard", "leaderboard.student_id", "=", "students.id")
    ...
    $current_user = DB::table('students')->select(DB::raw('sum(students_games.student_point) as total_points, students.id as id, users.profile_picture as profile_picture, users.name as name, users.school as school, users.state as state'))
      ->leftJoin("students_games", "students_games.student_id", "=", "students.id")
      ->join("users", "users.id", "=", "students.user_id")
